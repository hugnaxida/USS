 ...
 * @pre         
 *
 * Interrupt handler for interrupts of SYS_STATE module. It gets the modul's
...
{
  // PRQA S 0316 1
  sys_state_sInterruptEnvironmentData_t* evironmentData = (sys_state_sInterruptEnvironmentData_t*) nvic_GetPointerToEnvironmentData(nvic_IRQ_SYS_STATE);

  if ((irqsrc < sys_state_INTERRUPT_VECTOR_CNT) && (evironmentData != NULL))
  {
...
{  
  if ((vicIf != (const void *) NULL) && (vicIf->InterfaceVersion == NVIC_INTERFACE_VERSION) && (environmentdata != NULL))
  {
    ...
    vicIf->RegisterModule(nvic_IRQ_SYS_STATE, &loc_InterruptHandler, environmentdata);
    vicIf->EnableModule(nvic_IRQ_SYS_STATE);
  }
  else {}
