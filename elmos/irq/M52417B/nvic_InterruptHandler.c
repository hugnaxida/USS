...
/*  
* Exception:
* Msg(2:0305) [I] Cast between a pointer to function and an integral type.
*
* Circumstances and justification:
* Warning is issued when accessing the vector table in hardware and casting
* it to a local vector table representation.
* 
* Potential consequences:
* Underlying data structure. Uncontrolled memory access may occure.
*
* Alternative safety assurance:
* Check consistency by code review or during tests. IRQ will not work, if data
* access is handled wrongly.
*
*/
...
*
* Circumstances and justification:
* Vector table in hardware is casted to a data structure of IRQ environment
...
/*  
* Exception:
* Msg(2:0491) Array subscripting applied to an object of pointer type.
*
* Circumstances and justification:
* A part of the vector table in hardware is casted to a data structure of 
* IRQ environment data for easier access. The requested part of the vector table 
* is addressed by index.
* 
* Potential consequences:
* Underlying memory structure does not fit. Uncontrolled
* memory access may occure.
*
* Alternative safety assurance:
* Check consistency by code review or during tests. IRQ will not work, if data
...
/***************************************************************************//**
 * @implementation
 * The default exception handler disables the interrupt globally and
...
void nvic_RegisterModule(nvic_eExceptionVectorNum_t vno, nvic_InterruptCallback_t interrupthandler, nvic_pInterruptModuleEnvironmentData_t moduleenvironmentdata)
{
  ...
  {
    penvironmentdata->InterrupVectorTable[vno] = interrupthandler;  // TODO: eventually patch pointer to make it a THUMB ISA vector! (if not done by compiler)
