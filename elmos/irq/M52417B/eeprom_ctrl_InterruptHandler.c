/* ------------------------------------------------------------------- */
/** @file eeprom_ctrl_InterruptHandler.c
 *
...
#include "eeprom_ctrl_InterruptHandler.h"
...
/***************************************************************************//**
 * Handles the EEPROM related interrupt requests.
 *
 * @param       void
 *
 * @return      void
 ...
 * @pre         
 *
 * Interrupt handler for interrupts of EEPROM module. It gets the modul's
 ...
 * to eeprom_ctrl_SetIRQVEnable, that a special enum (eeprom_ctrl_eInterruptVectorNum_t)
...
{
  EEPROM_CTRL_IRQ_VENABLE = (uint16_t) irqsrc;
}

/***************************************************************************//**
 * @implementation
 * Simple word write access to IRQ_VDISABLE register, but with the difference
 * to eeprom_ctrl_SetIRQVDisable, that a special enum (eeprom_ctrl_eInterruptVectorNum_t)
 * is passed as parameter, for security reasons and a more concrete interface
 * definition.
 *
 ******************************************************************************/
...
{
  EEPROM_CTRL_IRQ_VDISABLE = (uint16_t) irqsrc;
}
...
{
  // PRQA S 0316 1
  eeprom_ctrl_sInterruptEnvironmentData_t* evironmentData = (eeprom_ctrl_sInterruptEnvironmentData_t*) nvic_GetPointerToEnvironmentData(nvic_IRQ_EEPROM_CTRL);

  if ((irqsrc < eeprom_ctrl_INTERRUPT_VECTOR_CNT) && (evironmentData != NULL))
  {
...
/***************************************************************************//**
 * @implementation
 * Reverse function of "eeprom_ctrl_RegisterInterruptCallback". Sets the corresponding
...
void eeprom_ctrl_InterruptDeregisterCallback(eeprom_ctrl_eInterruptVectorNum_t irqvecnum)
{
  ...
  if ((irqvecnum < eeprom_ctrl_INTERRUPT_VECTOR_CNT) && (evironmentData != NULL))
  {
    eeprom_ctrl_InterruptDisable(irqvecnum);
...
/***************************************************************************//**
 * @implementation
 * Every hardware module has it's own interrupt handler, also because each
 ...
 * (eeprom_ctrl_RegisterInterruptCallback) is copied into a local function pointer,
 ...
 ******************************************************************************/
 static __irq void loc_InterruptHandler(void)
{
  eeprom_ctrl_eInterruptVectorNum_t irqvecnum = (eeprom_ctrl_eInterruptVectorNum_t) EEPROM_CTRL_IRQ_VNO;
  
  if (irqvecnum < eeprom_ctrl_INTERRUPT_VECTOR_CNT) /* ensure the IRQ trigger is not already gone away until processing starts */
  {

    // PRQA S 0316 1
    eeprom_ctrl_sInterruptEnvironmentData_t* evironmentData = (eeprom_ctrl_sInterruptEnvironmentData_t*) nvic_GetPointerToEnvironmentData(nvic_IRQ_EEPROM_CTRL);

    eeprom_ctrl_InterruptCallback_t fptr;
...
//      __disable_interrupt();
    ...
    else 
    {
      ...
      EEPROM_CTRL_IRQ_VDISABLE = (uint16_t) irqvecnum;
    }
      
    /* Clear interrupt request flag */
    EEPROM_CTRL_IRQ_VNO = (uint16_t) irqvecnum;
  }
  else {}
...
{  
  if ((vicIf != (const void *) NULL) && (vicIf->InterfaceVersion == NVIC_INTERFACE_VERSION) && (environmentdata != NULL))
  {
    ...
    vicIf->RegisterModule(nvic_IRQ_EEPROM_CTRL, &loc_InterruptHandler, environmentdata);
    vicIf->EnableModule(nvic_IRQ_EEPROM_CTRL);
  }
  else {}
