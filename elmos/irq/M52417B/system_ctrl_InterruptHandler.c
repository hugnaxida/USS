/* ------------------------------------------------------------------- */
/** @file sys_ctrl_InterruptHandler.c
 *
...
/***************************************************************************//**
 * Handles the SYS_CTRL related interrupt requests.
 *
 * @param       void
 *
 * @return      void
 ...
 * @pre         
 *
 * Interrupt handler for interrupts of SYS_CTRL module. It gets the modul's
 ...
 * to system_ctrl_SetIRQVEnable, that a special enum (system_ctrl_eInterruptVectorNum_t)
...
{
  SYSTEM_CTRL_IRQ_VENABLE = (uint16_t) irqsrc;
}

/***************************************************************************//**
 * @implementation
 * Simple word write access to IRQ_VDISABLE register, but with the difference
 * to system_ctrl_SetIRQVDisable, that a special enum (system_ctrl_eInterruptVectorNum_t)
 * is passed as parameter, for security reasons and a more concrete interface
 * definition.
 *
 ******************************************************************************/
...
{
  SYSTEM_CTRL_IRQ_VDISABLE = (uint16_t) irqsrc;
}
...
{
  // PRQA S 0316 1
  system_ctrl_sInterruptEnvironmentData_t* evironmentData = (system_ctrl_sInterruptEnvironmentData_t*) nvic_GetPointerToEnvironmentData(nvic_IRQ_SYSTEM_CTRL);

  if ((irqsrc < system_ctrl_INTERRUPT_VECTOR_CNT) && (evironmentData != NULL))
  {
...
/***************************************************************************//**
 * @implementation
 * Reverse function of "system_ctrl_RegisterInterruptCallback". Sets the corresponding
...
void system_ctrl_InterruptDeregisterCallback(system_ctrl_eInterruptVectorNum_t irqvecnum)
{
  ...
  if ((irqvecnum < system_ctrl_INTERRUPT_VECTOR_CNT) && (evironmentData != NULL))
  {
    system_ctrl_InterruptDisable(irqvecnum);
...
/***************************************************************************//**
 * @implementation
 * Every hardware SYS_CTRL module has it's own interrupt handler, also because each
 ...
 * (system_ctrl_RegisterInterruptCallback) is copied into a local function pointer,
 ...
 ******************************************************************************/
 static __irq void loc_InterruptHandler(void)
{
  system_ctrl_eInterruptVectorNum_t irqvecnum = (system_ctrl_eInterruptVectorNum_t) SYSTEM_CTRL_IRQ_VNO;
  
  if (irqvecnum < system_ctrl_INTERRUPT_VECTOR_CNT) /* ensure the IRQ trigger is not already gone away until processing starts */
  {

    // PRQA S 0316 1
    system_ctrl_sInterruptEnvironmentData_t* evironmentData = (system_ctrl_sInterruptEnvironmentData_t*) nvic_GetPointerToEnvironmentData(nvic_IRQ_SYSTEM_CTRL);

    system_ctrl_InterruptCallback_t fptr;
...
//      __disable_interrupt();
    ...
    else 
    {
      ...
      SYSTEM_CTRL_IRQ_VDISABLE = (uint16_t) irqvecnum;
    }
      
    /* Clear interrupt request flag */
    SYSTEM_CTRL_IRQ_VNO = (uint16_t) irqvecnum;
  }
  else {}
...
{  
  if ((vicIf != (const void *) NULL) && (vicIf->InterfaceVersion == NVIC_INTERFACE_VERSION) && (environmentdata != NULL))
  {
    ...
    vicIf->RegisterModule(nvic_IRQ_SYSTEM_CTRL, &loc_InterruptHandler, environmentdata);
    vicIf->EnableModule(nvic_IRQ_SYSTEM_CTRL);
  }
  else {}
