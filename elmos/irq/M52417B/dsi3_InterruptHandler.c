...
/***************************************************************************//**
 * Handles the DSI3 related interrupt requests.
 *
 * @param       void
 *
 * @return      void
 ...
 * @pre         
 *
 * Interrupt handler for interrupts of DSI3 module. It gets the modul's
 ...
 *
 ******************************************************************************/
static __irq void loc_dsi3_InterruptHandler(void);
 ...
 * to dsi3_SetIRQVEnable, that a special enum (dsi3_eInterruptVectorNum_t)
...
{
  DSI3_IRQ_VENABLE = (uint16_t) irqsrc;
}

/***************************************************************************//**
 * @implementation
 * Simple word write access to IRQ_VDISABLE register, but with the difference
 * to dsi3_SetIRQVDisable, that a special enum (dsi3_eInterruptVectorNum_t)
 * is passed as parameter, for security reasons and a more concrete interface
 * definition.
 *
 ******************************************************************************/
...
{
  DSI3_IRQ_VDISABLE = (uint16_t) irqsrc;
}
...
{
  // PRQA S 0316 1
  dsi3_sInterruptEnvironmentData_t* evironmentData = (dsi3_sInterruptEnvironmentData_t*) nvic_GetPointerToEnvironmentData(nvic_IRQ_DSI3);
  
  if ((irqsrc < dsi3_INTERRUPT_VECTOR_CNT) && (evironmentData != NULL)) 
  {
...
/***************************************************************************//**
 * @implementation
 * Reverse function of "dsi3_RegisterInterruptCallback". Sets the corresponding
...
void dsi3_InterruptDeregisterCallback(dsi3_eInterruptVectorNum_t irqvecnum)
{
  ...
  if ((irqvecnum < dsi3_INTERRUPT_VECTOR_CNT) && (evironmentData != NULL)) 
  {
    dsi3_InterruptDisable(irqvecnum);
...
/***************************************************************************//**
 * @implementation
 * Every hardware DSI3 module has it's own interrupt handler, also because each
 ...
 * dsi3_InterruptHandler. Next the registered interrupt callback function
 * (dsi3_RegisterInterruptCallback) is copied into a local function pointer,
...
static __irq void loc_dsi3_InterruptHandler(void)
{
  dsi3_eInterruptVectorNum_t irqvecnum = (dsi3_eInterruptVectorNum_t) DSI3_IRQ_VNO;

  if (irqvecnum < dsi3_INTERRUPT_VECTOR_CNT) /* ensure the IRQ trigger is not already gone away until processing starts */
  {  
    // PRQA S 0316 1
    dsi3_sInterruptEnvironmentData_t* evironmentData = (dsi3_sInterruptEnvironmentData_t*) nvic_GetPointerToEnvironmentData(nvic_IRQ_DSI3);

    dsi3_InterruptCallback_t fptr;
...
//      __disable_interrupt();    
    ...
    else 
    {
      ...
      DSI3_IRQ_VDISABLE = (uint16_t) irqvecnum;
    }
  
    /* Clear interrupt request flag */
    DSI3_IRQ_VNO = (uint16_t) irqvecnum;
  }
  else {}
...
void dsi3_InterruptInitialisation(nvic_cpInterfaceFunctions_t vicIf, dsi3_pInterruptEnvironmentData_t environmentdata, dsi3_pInterruptContextData_t contextdata)
{
  if (( vicIf !=  (const void *) NULL) && (vicIf->InterfaceVersion == NVIC_INTERFACE_VERSION) && (environmentdata != NULL))
  {
    ...
    vicIf->RegisterModule(nvic_IRQ_DSI3, &loc_dsi3_InterruptHandler, environmentdata);
    vicIf->EnableModule(nvic_IRQ_DSI3);
  }
  else {}
