 ...
 *
 ******************************************************************************/
void gpio_InterruptRegisterCallback(gpio_eInterruptVectorNum_t irqsrc, gpio_InterruptCallback_t cbfnc)
{
  // PRQA S 0316 1
  gpio_sInterruptEnvironmentData_t* evironmentData = (gpio_sInterruptEnvironmentData_t*) nvic_GetPointerToEnvironmentData(nvic_IRQ_GPIO);

  if ((irqsrc < gpio_INTERRUPT_VECTOR_CNT) && (evironmentData != NULL)) 
  {
 ...
 *
 ******************************************************************************/
void gpio_InterruptInitialisation(nvic_cpInterfaceFunctions_t vicIf, gpio_pInterruptEnvironmentData_t environmentdata, gpio_pInterruptContextData_t contextdata)
{  
  if ((vicIf != (const void *) NULL) && (vicIf->InterfaceVersion == NVIC_INTERFACE_VERSION) && (environmentdata != NULL))
  {
    ...
    vicIf->RegisterModule(nvic_IRQ_GPIO, &loc_InterruptHandler, environmentdata);
    vicIf->EnableModule(nvic_IRQ_GPIO);
  }
  else {}
