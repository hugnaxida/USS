 ...
 * $Id: Dsi3Bus_Implementation.c 422 2019-03-14 14:55:33Z poe $
 *
 * @verbatim
 * -------------------------------------------------------------------
 * Date     B/F Who  Ver  Comment
 * -------------------------------------------------------------------
 * 19/03/14 (F) POE  1029 Removed lower and upper boundary checks for CRM response offset during bus initialization.
 * 19/02/25 (B) POE  1028 Fixed DSI3 autosync side effect caused by task clk_1us update
                          CRM BRCs & non targeted CRMs also trigger the CRMProcessedCallback now.
 * 18/12/04 (F) POE  1027 Added manual start of current measurement during discovery mode to TimerA (E524.17A1)
 * 18/11/13 (B) POE  1026 Added fix implementations for ESWDSI3-16,-17,-18.
 * 18/09/24 (B) POE  1025 Added fix implementation for DEN524XX-471.
 * 18/09/06 (F) POE  1024 Added bugfix for ESWDSI3-12 & -13.
 * 18/09/06 (F) POE  1023 Synchronized with M52417B Bus implementation.
 * 18/05/09 (F) POE  1022 Added check for DSI3 HW Version; Added precompilerflag for DSI3_MAX_NUMBER_OF_CALLBACK_TABLES==1
 *                        to remove warning message
 * 18/04/23 (F) POE  1021 Replaced DSI3BUSIMP_TIMER_X_MAX_VALUE with DSI3_TIMERX_MAX_VALUE reference
 * 18/04/18 (F) POE  1020 Removed CRMResponseTimeOffset from BlankingTime calculation.
 * 18/04/17 (F) POE  1019 ChangeMode now enforces a mode re-init if applied on the same DSI3 mode.
 * 18/04/06 (F) POE  1018 Added check during PDCM irq handling for TimerC configuration value and
 *                        switches blanking off if configured value is < current DSI3 timer value.
 * 18/04/05 (F) POE  1017 Removed old change comments. Removed AddHWCRC If function.
 * 18/03/22 (F) POE  1016 Replaced bus activity check of fcc_low_flag with a software based flag solution.
 * 18/03/21 (B) POE  1015 Removed PrepareNextCommState calls on FCCErr, StartErr & CRCErr.
 *                        Removed precompiler flag in the initialization.
 *                        Added blanking time offset substraction from Disc period to prevent FCC errors.
 * 18/03/20 (B) POE	 1014 - Reactivated PDCM Evt handling to activate auto response after mode change to PDCM.
 * 18/03/14 (F) POE  1013 - Removed precompiler flags for CRM based autoaddressing. 
 *                        - SlaveId filtering is now configurable during init process.
 *                        - After notifying the upper layer bus state machine switches to idle
 *                          to be able to update to new slave id before the end of the full discovery
 *                          mode processing.
 *                        - Removed obsolete debug mode precompiler flag.
 * 18/03/01 (B) POE  1012 Set PDCM default response to ignore. Moved response handling into callback if block.
 * 18/02/26 (F) POE  1011 Added multi cb table support and if funs;Added PDCM clk sync fixes (ESWDSI3-3 & ESWDSI3-4).
 * 18/02/22 (F) POE  1010 Added IF fnc for fcc low flag check;Removed remaining PDCMSyncCb segments.
 * 18/02/21 (B) POE  1009 Added disabling of FCC blanking on RCC_FINISH & CRCError.
 * 18/02/09 (F) POE  1008 Added missing DISCDelayErrEvtHandling assignment in init()
 * 18/02/08 (F) POE  1007 Implemented switchable delayed handling of PDCM/DISCErr
 * 18/02/01 (B) POE  1006 Enabled FCCErrEvt handling for PDCM & DISC modes
 * 18/02/01 (B) JNAU 1005 Removed QA-C and CodeSonar warnings
 * 18/01/22 (F) POE  1004 replaced DSI3_CTRL_FCC_CMD_READ() with DSI3_CTRL_FCC_CMD_READ(val) to
 *                        prevent compiler warning                      
 * 18/01/19 (F) POE  1003 added timer reset and blanking off to FCCErrEvt & StartErrEvt handler.
...
BUS_LAYER void Dsi3BusImp_InterruptHandler_TimerB_Evt(dsi3_eInterruptVectorNum_t sno, Dsi3BusIf_pGenericCbCtxData_t genericCbCtxData)
{
  ...
  if (DSI3_NULL != genericCbCtxData)
  {
    ...
    if (Dsi3BusIf_eCOMM_DISCOVERYMODE == busEnvData->Steering.CurrentCommunicationMode)
    {
       ...
       * hardware register */
      //if (FALSE == DSI3_DISC_CTRL_CURR_CMP_OUT_CAP_GET())
      ...
      {        
        ...
        DSI3_DISC_CTRL_AUTO_DISC_RESP_DISABLE();
          /* --> 18/09/19, poe: DEN524XX-471
           * Moved to the start of the handler. 
           */        
//        /* --> 2018-08-06, poe: ESWDSI3-14
//         * Once addressed, DiscCurr irq is not needed anymore.
//         */
//        /* Disable CurrCmp IRQ */
//        DSI3_CTRL_IRQ_DISABLE(dsi3_IRQ_DISC_CURR_EVT);
//        /* --> 2018-08-06, poe: ESWDSI3-14 */
        /* Disable TimerB IRQ */
        DSI3_CTRL_IRQ_DISABLE(dsi3_IRQ_TIMER_B_EVT);
        /* --> 2018-09-18, poe:  DEN524XX-471
		 * Re-enabled TimerA handler, so it must be disabled again.
		 */
        ...
        /* Return to idle mode - addressing is primarily done now */
        busEnvData->Steering.Dsi3State = Dsi3BusIf_eSTATE_IDLE;
      }
      else{ /* Do nothing */ }
      /* --> 2018-09-18, poe:  DEN524XX-471
                                                      ...
                                                      Dsi3BusIf_cpGenericImpCfgData_t   genericImpCfgData)
{
  ...
  if (DSI3_NULL != (void *) genericBusEnvData)
  {
      ...
      && (DSI3_IMPL_TARGET_HARDWARE_VERSION == DSI3_HW_VERSION))
    {
      ...
      DSI3_CONFIG                                 = 0u;
	  DSI3_CONFIG                                 = E_DSI3_CONFIG_CRC_CHECK 
                                                  | (dsi3Cfg->HardwareAddsCRC * E_DSI3_CONFIG_CRC_ADD)
                                  //              | E_DSI3_CONFIG_AUTO_RESP
                                                  | E_DSI3_CONFIG_AUTO_TIMER
                                                  | (dsi3Cfg->EnableAutoSync * E_DSI3_CONFIG_AUTO_SYNC)
                                    //            | E_DSI3_CONFIG_DISABLE_SYNC_VALI
                                                  | E_DSI3_CONFIG_AUTO_BLANK_ON
                                                  | E_DSI3_CONFIG_AUTO_BLANK_OFF
                                                  | (busEnvData->Steering.ChipTime<<SFT_DSI3_CONFIG_T_CHIP)
                                                  | (dsi3_ePDCM_Seed_ID<<SFT_DSI3_CONFIG_PDCM_SEED_SEL)
                                                  | (dsi3Cfg->Dsi3DebounceValue<<SFT_DSI3_CONFIG_DEBOUNCE)
                                                  | (DSI3_TIMER_ST_EDGE<<SFT_DSI3_CONFIG_TIMER_START_SEL);
