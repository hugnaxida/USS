...
{
  UART_IRQ_VENABLE = (uint16_t) irqsrc;  
}
/***************************************************************************//**
 * @implementation
...
{
  UART_IRQ_VDISABLE = (uint16_t) irqsrc;
}
/***************************************************************************//**
 * @implementation
...
{
  // PRQA S 0316 1
  uart_sInterruptEnvironmentData_t* evironmentData = (uart_sInterruptEnvironmentData_t*) nvic_GetPointerToEnvironmentData(nvic_IRQ_UART);

  if ((irqsrc < uart_INTERRUPT_VECTOR_CNT) && (evironmentData != NULL))
  {
...
void uart_InterruptDeregisterCallback(uart_eInterruptVectorNum_t irqvecnum)
{
  ...
  if ((irqvecnum < uart_INTERRUPT_VECTOR_CNT) && (evironmentData != NULL))
  {
 ...
 ******************************************************************************/
 static __irq void loc_InterruptHandler(void)
{
  uart_eInterruptVectorNum_t irqvecnum = (uart_eInterruptVectorNum_t) UART_IRQ_VNO;
...
//      __disable_interrupt();
    ...
    else 
    {
      ...
      UART_IRQ_VDISABLE = (uint16_t) irqvecnum;
    }
      
    /* Clear interrupt request flag */
    UART_IRQ_VNO = (uint16_t) irqvecnum;
...
{  
  if ((vicIf != (const void *) NULL) && (vicIf->InterfaceVersion == NVIC_INTERFACE_VERSION) && (environmentdata != NULL))
  {
    ...
    vicIf->RegisterModule(nvic_IRQ_UART, &loc_InterruptHandler, environmentdata);
    vicIf->EnableModule(nvic_IRQ_UART);
  }
  else {}
