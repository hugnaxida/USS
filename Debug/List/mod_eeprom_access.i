...
{
  return ( ( addr >= 0x48000000u ) && ( addr <= ( (0x48000000u) + (0x0100u) - 1 ) ) ? 1 : 0 );
}
...
{
  (((EEPROM_CTRL_Type *) 0x40000500UL)->LOCK_CNT . reg) = 0;
  ...
  while (unlockcnt > 0) 
  {  
    (((EEPROM_CTRL_Type *) 0x40000500UL)->LOCK_CNT . reg) = pass;  // 9x -> ready for LOCK_L/LOCK_U changes (if unlockcnt properly set to 9)
    --unlockcnt;
  }  
  
  (((EEPROM_CTRL_Type *) 0x40000500UL)->LOCK_L . reg) = lock_l | masked_pass;
  (((EEPROM_CTRL_Type *) 0x40000500UL)->LOCK_U . reg) = lock_u | masked_pass;
  
  /* Push EEPROM state machine forward. */
  (((EEPROM_CTRL_Type *) 0x40000500UL)->LOCK_CNT . reg) = pass; // 10
  (((EEPROM_CTRL_Type *) 0x40000500UL)->LOCK_CNT . reg) = pass; // 11  -> ready for LOCK_U_FREEZE changes 

  (((EEPROM_CTRL_Type *) 0x40000500UL)->LOCK_L_FREEZE . reg) = lock_l_freeze | masked_pass;
  (((EEPROM_CTRL_Type *) 0x40000500UL)->LOCK_U_FREEZE . reg) = lock_u_freeze | masked_pass;

  /* Push EEPROM state machine forward. */
  (((EEPROM_CTRL_Type *) 0x40000500UL)->LOCK_CNT . reg) = pass; // 12  -> ready for MODE changes 
  
  (((EEPROM_CTRL_Type *) 0x40000500UL)->MODE . reg) = mode | masked_pass; 
  
  /* Push EEPROM state machine forward. */
  (((EEPROM_CTRL_Type *) 0x40000500UL)->LOCK_CNT . reg) = pass; // 13 ->  ready for access
}
...
static bool_t loc_WriteDoubleWord(eeprom_Password_t pass, eeprom_Address_t dptr, uint16_t datalow, uint16_t datahigh, uint8_t mode, eeprom_LockBits_t lock_l, eeprom_LockBits_t lock_u)
{
  ...
  if ( eeprom_ResetIP() != 1 ) /* optionally try to reset and reenable MTP IP, if needed */ 
  {
    return 0; 
  }
  else {}
  ...
  *dptr = datalow;
  ...
  while ( ( (((EEPROM_CTRL_Type *) 0x40000500UL)->STATUS . reg) & 0x01u ) == 1u  ) { }; /* wait as long as busy, EEPROM_CTRL will timeout automatically, in case of IP failure */
  
  if (mode != (0x0001u)) /* if not erasing write second word */ 
  {
    *(dptr+1) = datahigh;
    __iar_builtin_DSBx(15);
...
static bool_t loc_MarginReadDoubleWord(eeprom_Password_t pass, eeprom_Address_t dptr, uint16_t prechargedatalow, uint16_t prechargedatahigh, uint16_t *datalow, uint16_t *datahigh)
{
  ...
  loc_ConfEEStateMachine(pass,unlockcnt,(0x0004u),0,0,0,0);
...
eeprom_Bool_t eeprom_CheckIPState(void)
{
  if ( ( ( (((EEPROM_CTRL_Type *) 0x40000500UL)->STATUS . reg) & 0x01u ) == 0u ) && ( (((EEPROM_CTRL_Type *) 0x40000500UL)->IRQ_STATUS . reg) == 0x0000u ) ) // if busy flag is gone, and no errors have occured MTP is up and running. 
  { 
		return 1; // IP in a sane state... 
  }
  else
  {
...
eeprom_Bool_t eeprom_ResetIP(void)
{  
  if ( ( (((EEPROM_CTRL_Type *) 0x40000500UL)->STATUS . reg) & 0x01u ) == 1 ) /* if status is already "1" some previous access has failed and locked the EEPROM_CTRL, this will likely not change anymore, so try reset IP, if possible */ 
  {
    (((EEPROM_CTRL_Type *) 0x40000500UL)->IP_ENABLE . reg) = 0xA500u; // disable MTP IP core.		
    (((EEPROM_CTRL_Type *) 0x40000500UL)->IP_ENABLE . reg) = 0xA501u; // reenable MTP IP core.

    while ( ( (((EEPROM_CTRL_Type *) 0x40000500UL)->STATUS . reg) & 0x01u ) == 1  ) { }; /* will need some cycles until IP gets ready again ... wait as long as busy set */
  }
    
  /* reset IRQ flags in any case, to be able to use eeprom_CheckIPState() once further access have been performed */
  (((EEPROM_CTRL_Type *) 0x40000500UL)->IRQ_MASK . reg)   = 0x0000; // disable all EE IRQs
  (((EEPROM_CTRL_Type *) 0x40000500UL)->IRQ_STATUS . reg) = (0x0001u)
                         | (0x0002u)
                         | (0x0004u);  // clear all IRQ status flags.
    
  return 1; // MTP should be in a sane state now.     
}
...
void eeprom_GetLock(eeprom_LockBits_t *lock_l, eeprom_LockBits_t *lock_u)
{
  if (lock_l != ((void *)0))
  {
    *lock_l = ((((EEPROM_CTRL_Type *) 0x40000500UL)->LOCK_L . reg) & 0xffu);
  }
  else{}

  if (lock_u != ((void *)0))
  {
    *lock_u = ((((EEPROM_CTRL_Type *) 0x40000500UL)->LOCK_U . reg) & 0xffu);
  }
  else{}
}
...
{    
  ...
  */
  if ( (sptrbyte == ((void *)0)) ||
       (loc_isInsideEE((uintptr_t)sptrbyte) != 0) ||
       (loc_isInsideEE(((uintptr_t)sptrbyte) + (bytes - 1)) != 0))
  {
    return(0);
  }
  else {}
  ...
  */
  if ( (dptr == ((void *)0)) ||
       (((uintptr_t)dptr & 3u) != 0) ||
       (loc_isInsideEE((uintptr_t)dptr) == 0) ||
       (loc_isInsideEE(((uintptr_t)dptr) + (bytes - 1)) == 0))
  {
    return(0);
  }
  else {}
  ...
  while (bytes>0)
  {
    ...
    {    
      ...
      if (loc_MarginReadDoubleWord(pass,dptr,0x0000u,0x0000u,&readbackLowerBoundLowWord,&readbackLowerBoundHighWord) != 1)
      { --retrycount; continue; }
      else {} 
      if (loc_MarginReadDoubleWord(pass,dptr,0xFFFFu,0xFFFFu,&readbackdatalow,&readbackdatahigh) != 1)
      { --retrycount; continue; }
      else {} 
      
      /* check vsup before writing on eeprom */
      if (app_CheckVsup() < 182u)
      { 
        --retrycount; continue; 
      }
      else {}
      ...
      { 
        ...
        if (loc_WriteDoubleWord(pass,dptr,0x0000u,0x0000u,(0x0001u),lock_l,lock_u) != 1)           
        { --retrycount; continue; }
        else {}
      }              
      ...
      if (loc_WriteDoubleWord(pass,dptr,srcdatalow,srcdatahigh,(0x0002u),lock_l,lock_u) != 1)      
      { --retrycount; continue; }
      else {}      
      
      /* check written data is valid */
      if (loc_MarginReadDoubleWord(pass,dptr,srcdatalow,srcdatahigh,&readbackdatalow,&readbackdatahigh) != 1)
      { --retrycount; continue; }
      else {}      
      
      /* check if read lead to a failure on MTP-IP level. (checks state of state-machine) */
      if ( eeprom_CheckIPState() == 0 ) 
      { --retrycount; continue; }
      else {}      
...
eeprom_Bool_t eeprom_Erase(eeprom_Password_t pass, eeprom_Address_t dptr, eeprom_Length_t bytes, eeprom_LockBits_t lock_l, eeprom_LockBits_t lock_u)
{  
  static const uint8_t zero_buf[4] = { 0x0000u, 0x0000u, 0x0000u, 0x0000u };
  ...
  while(bytes>0)
  {
    if (eeprom_Write(pass, zero_buf, dptr, sizeof(zero_buf), lock_l, lock_u) != 1) 
    { return(0); }
    else {}    
...
eeprom_Bool_t eeprom_Initialize(eeprom_MarginReadTrim_t mrtrim)
{
  if (mrtrim > 8) { return 0; }
  else {}
  
  if ( eeprom_ResetIP() != 1 ) { return 0; }
  else {}
    
  (((EEPROM_CTRL_Type *) 0x40000500UL)->MARGIN_TRIM . reg) = 0xA500u | ( mrtrim & 0x0007u );
