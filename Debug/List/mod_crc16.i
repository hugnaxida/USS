...
void crc16_Init(void)
{
  ...
  (((CRC16_Type *) 0x40000200UL)->CONFIG . bit).keep_sum = 0u;
  /* divide clock for CRC hardware by 2^timing, to not block the memory */
  (((CRC16_Type *) 0x40000200UL)->CONFIG . bit).timing = 0u;
}
...
bool_t crc16_checkCrc(uint32_t startAddress, uint32_t numBytes, uint16_t expectedCrc, uint16_t* returnCrc)
{
  ...
  do
  {
    crcState = (((CRC16_Type *) 0x40000200UL)->STATUS . bit).state;
  }
  while((0u) == crcState);

  /* write expected CRC to crc16 register */
  (((CRC16_Type *) 0x40000200UL)->EXPECTED_CRC . bit).crc = expectedCrc;
  /* write number of bytes which have to be controlled into crc16 register */
  (((CRC16_Type *) 0x40000200UL)->LENGTH1 . bit).bytes = (numBytes & 0xFFFF0000u) >> 16u;  /* msb */
  (((CRC16_Type *) 0x40000200UL)->LENGTH0 . bit).bytes = numBytes & 0x0000FFFFu;          /* lsb */
  /* write lsb of memory address for crc checking to crc16 register */
  (((CRC16_Type *) 0x40000200UL)->START0 . bit).addr = ((uint32_t)startAddress) & 0x0000FFFFu;
  /* write msb of memory address for crc checking to crc16 register, which starts the crc calculation */
  (((CRC16_Type *) 0x40000200UL)->START1 . bit).addr = (((uint32_t)startAddress) & 0xFFFF0000u) >> 16u;
  ...
  if(((void *)0) != returnCrc)
  {
    *returnCrc = (((CRC16_Type *) 0x40000200UL)->CRC_SUM . bit).crc;
  }
  else
  {
    /* nothing to do here */
