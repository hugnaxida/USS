...
void mem_Task(void)
{
  if(((void *)0) != loc_pAppDataMem)
  {
    if(1 == calcCrcOverRamParam)
    {
      /* Calculate the CRC over the RAM area, which contains the copy of the EEPROM config data */
      (void)crc16_checkCrc((uint32_t)loc_pAppDataMem , (198u), 0, &(loc_pAppDataMem->data.CrcCalculation.fields.CrcEepromOrRam));
      calcCrcOverRamParam = 0;
    }
    else
    {
  ...
  {
    /* init not successful -> goto error handler */
    err_ErrorHandlerExt(133, ERR_MODLIST_MEM);;
  }
}
