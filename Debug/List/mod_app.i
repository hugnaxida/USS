...
bool_t app_Init(Dsi3Drv_pApplCtrlData_t pApplRuntimeData)
{
  ...
  if(((void *)0) != pApplRuntimeData)
  {
    ...
    ((((DSP_Type *) 0x50000400UL)->EEVAL_DMA_CTRL . bit). wrap_off = (1u));                        /* disable wrap around */
    ((((DSP_Type *) 0x50000400UL)->EEVAL_DMA_CTRL . bit). length = (uint8_t)(0u));           /* set buffer size to zero, to disable dma until it is needed */
    ((((DSP_Type *) 0x50000400UL)->EEVAL_DMA_ADDR . reg) = (uint16_t)(util_calculateDmaMemoryOffset((void*)pApplRuntimeData->MeasCtrl->Results . resultBuf . raw))); /* set pointer to standard measurement ringbuffer */
    /* DMA for advanced measurement */
    ((((DSP_Type *) 0x50000400UL)->EEVAL1_DMA_CTRL . bit). wrap_off = (1u));                       /* disable wrap around */
    ((((DSP_Type *) 0x50000400UL)->EEVAL1_DMA_CTRL . bit). length = (uint8_t)(0u));          /* set buffer size to zero, to disable dma until it is needed */
    ((((DSP_Type *) 0x50000400UL)->EEVAL1_DMA_ADDR . reg) = (uint16_t)(util_calculateDmaMemoryOffset((void*)pApplRuntimeData->MeasCtrl->Results . resultBuf . raw))); /* set pointer to standard measurement ringbuffer */
    ((((DSP_Type *) 0x50000400UL)->EEVAL2_DMA_CTRL . bit). wrap_off = (1u));                       /* disable wrap around */
    ((((DSP_Type *) 0x50000400UL)->EEVAL2_DMA_CTRL . bit). length = (uint8_t)(0u));          /* set buffer size to zero, to disable dma until it is needed */
    ((((DSP_Type *) 0x50000400UL)->EEVAL2_DMA_ADDR . reg) = (uint16_t)(util_calculateDmaMemoryOffset((void*)(&pApplRuntimeData->MeasCtrl->Results . resultBuf . adv[(20u)])))); /* set pointer to standard measurement ringbuffer */
...
//    retval = retval && dsp_RegisterCustomInterruptCallback(dsp_IRQ_EEVAL_MIN_EVT, loc_MeasEvtOccuredInPath1);
    ...
    ((((DSP_Type *) 0x50000400UL)->IRQ_MASK . reg) |= (1u << (dsp_eInterruptVectorNum_t)(dsp_IRQ_EEVAL_DMA_WRAP)));
    ((((DSP_Type *) 0x50000400UL)->IRQ_MASK . reg) |= (1u << (dsp_eInterruptVectorNum_t)(dsp_IRQ_EEVAL1_DMA_WRAP)));
    ((((DSP_Type *) 0x50000400UL)->IRQ_MASK . reg) |= (1u << (dsp_eInterruptVectorNum_t)(dsp_IRQ_EEVAL2_DMA_WRAP)));
    
    /* Calculate slope of temperature sensor curve with calibration value with 0,01*1024 = 10*/
    tsense_calib_eff = (((-589) * (1024 + ((int32_t)((((*((uint16_t*)((0x48000000u) + 0xF8u)))&0xFC00u)>>10u) - (32u)) *10))) >> 10);
  }
  else
  {
    retval = 0;
    err_ErrorHandlerExt(284, ERR_MODLIST_APP);;
  }
  return retval;
}
...
void app_PdcmCmdHandler_APP_MODE(Dsi3Drv_uPDCMResponseFrame_t* ansData)
{
  if((((void *)0) != ansData) && (((void *)0) != loc_pApplRuntimeData))
  {
    ...
    Meas_sEevalAdvDataPacketFormat*   loc_BufAdv2 = &loc_pApplRuntimeData->MeasCtrl->Results.resultBuf.adv[(20u)];

    /* set default data */
    loc_SetPdcmAnswerFrameDefault(ansData);

    /* check if diagnostic measurement has finished */
    if(1 == loc_pApplRuntimeData->MeasCtrl->Results.DiagnosticMeasurement.AccquisitionFinished)
    {
      /* only send diagnostic measurement results of not already send */
      if(0 == loc_pApplRuntimeData->MeasCtrl->Results.DiagnosticMeasurement.ResultsReadout)
      {
        ...
        loc_pApplRuntimeData->MeasCtrl->Results.DiagnosticMeasurement.ResultsReadout = 1;
      }
      else
      {
        ...
        if((meas_eMeasConf_INDIRECT != loc_pApplRuntimeData->MeasCtrl->RuntimeData.MeasurementConf) &&
           ((0 == loc_pApplRuntimeData->MeasCtrl->Results.BurstMonitoring.ResultsReadout) || (0 == loc_pApplRuntimeData->MeasCtrl->Results.RingingBehaviour.ResultsReadout)))
        {
          /* check if burst monitoring has finished */
          if(1 == loc_pApplRuntimeData->MeasCtrl->Results.BurstMonitoring.AccquisitionFinished)
          {
            /* only send burst monitoring results of not already send */
            if(0 == loc_pApplRuntimeData->MeasCtrl->Results.BurstMonitoring.ResultsReadout)
            {
              /* send result of <Burst monitoring> */
              loc_SetPdcmAnswerFrameBurst(ansData);
              /* indicate, that this data has been readout */
              loc_pApplRuntimeData->MeasCtrl->Results.BurstMonitoring.ResultsReadout = 1;
            }
            else
            {
              /* check if ringing behavior measurement has finished */
              if(1 == loc_pApplRuntimeData->MeasCtrl->Results.RingingBehaviour.AccquisitionFinished)
              {
                /* only send ringing behavior measurement results of not already send */
                if(0 == loc_pApplRuntimeData->MeasCtrl->Results.RingingBehaviour.ResultsReadout)
                {
                  /* send result of <Ringing behavior> */
                  loc_SetPdcmAnswerFrameRinging(ansData);
                  /* indicate, that this data has been readout */
                  loc_pApplRuntimeData->MeasCtrl->Results.RingingBehaviour.ResultsReadout = 1;
                }
                else
                {
                  /* this case should be impossible to reach*/
                  err_ErrorHandlerExt(358, ERR_MODLIST_APP);;
                }
              }
              else
              {
        ...
        else
        {
          ...
          if((1 == loc_pApplRuntimeData->MeasCtrl->Results.NearFieldDetection.Nfd1Finished) && (1 == loc_pApplRuntimeData->MeasCtrl->Results.NearFieldDetection.Nfd2Finished))
          {
            /* prepare answer buffer */
            loc_SetPdcmAnswerFrameNfd(ansData);
            /* prevent NFD from been send again */
            loc_pApplRuntimeData->MeasCtrl->Results.NearFieldDetection.Nfd1Finished = 0;
            loc_pApplRuntimeData->MeasCtrl->Results.NearFieldDetection.Nfd2Finished = 0;
          }
          else
          {
            ...
            if(meas_eMeasKind_STANDARD == loc_pApplRuntimeData->MeasCtrl->RuntimeData.MeasurementKind)
            {
              ...
              if(loc_pApplRuntimeData->MeasCtrl->Results.MeasRingbufTail < loc_pApplRuntimeData->MeasCtrl->Results.MeasRingbufHead)
              {
                ...
                if((0u) == (((DSP_Type *) 0x50000400UL)->EEVAL_CTRL . bit).eeval_dma_val)
                {
  ...
  else
  {
    ...
    err_ErrorHandlerExt(514, ERR_MODLIST_APP);;
  }
}
...
void app_ResetMeasurementFlagsCritical(Meas_sMeasurementResults_t* pResultBuf)
{
  if(((void *)0) != pResultBuf)
  {
    pResultBuf->BurstMonitoring.AccquisitionFinished = 0;
    pResultBuf->BurstMonitoring.ResultsReadout = 0;
    pResultBuf->DiagnosticMeasurement.AccquisitionFinished = 0;
    pResultBuf->DiagnosticMeasurement.ResultsReadout = 0;
    pResultBuf->RingingBehaviour.AccquisitionFinished = 0;
    pResultBuf->RingingBehaviour.ResultsReadout = 0;
    pResultBuf->NearFieldDetection.Nfd1Finished = 0;
    pResultBuf->NearFieldDetection.Nfd2Finished = 0;
  }
  else
  {
    /* called before init was executed correctly -> goto error handler */
    err_ErrorHandlerExt(545, ERR_MODLIST_APP);;
  }
}
...
void app_MeasurementInit(Meas_sMeasurementResults_t* pResultBuf)
{
  if(((void *)0) != pResultBuf)
  {
    ...
    ((((DSP_Type *) 0x50000400UL)->EEVAL_CTRL . bit). eeval_sel = 0u);
    ...
    pResultBuf->NearFieldDetection.Nfd1Flag = 0;
    pResultBuf->NearFieldDetection.Nfd2Flag = 0;
  ...
  {
    /* called before init was executed correctly -> goto error handler */
    err_ErrorHandlerExt(600, ERR_MODLIST_APP);;
  }
}
...
void app_DiagnosticMeasurement(Meas_sMeasurementResults_t* pResultBuf)
{
  ...
  {
    /* called before init was executed correctly -> goto error handler */
    err_ErrorHandlerExt(639, ERR_MODLIST_APP);;
  }
}
...
bool_t app_NoiseMeasurement(Dsi3Drv_pMeasCtrl_t pMeasCtrl)
{
  ...
  {
    /* get sample */
    uint32_t loc_envSample = ((((DSP_Type *) 0x50000400UL)->AMPD_ENV . bit). ampd_env);

    /* Collect all sample values for the Master (12 bit resolution)*/
    if((pMeasCtrl->Results.DiagnosticMeasurement.NoiseMeasNoiseSum + loc_envSample) > (4095u))
    {
      /* Maximum value is reached. Keep the maximum. */
      pMeasCtrl->Results.DiagnosticMeasurement.NoiseMeasNoiseSum = (4095u);
    }
    else
    {
    ...
    if(loc_envSample > pMeasCtrl->Parameters.data.NoiseMeasurement.fields.NoiseMeasThreshold)
    {
      ...
      if(pMeasCtrl->Results.DiagnosticMeasurement.NoiseMeasNoiseCount < (255u))
      {
      ...
      else
      {
        ...
        pMeasCtrl->Results.DiagnosticMeasurement.NoiseMeasNoiseCount = (255u);
      }
    }
    else
    {
    ...
    if((pMeasCtrl->Results.DiagnosticMeasurement.NoiseMeasSamplesTaken >= pMeasCtrl->Parameters.data.NoiseMeasurement.fields.NoiseMeasTime) ||
       (pMeasCtrl->Results.DiagnosticMeasurement.NoiseMeasSamplesTaken >= (255u)))
    {
      /* indicate, that diagnostic measurement has finished */
      pMeasCtrl->Results.DiagnosticMeasurement.AccquisitionFinished = 1;
  ...
  {
    /* called before init was executed correctly -> goto error handler */
    err_ErrorHandlerExt(718, ERR_MODLIST_APP);;
  }
  return retval;
}
...
void app_ActivateDmaForEchoMeas(Meas_eMeasurementKind_t measMode)
{
  if(((void *)0) != loc_pApplRuntimeData)
  {
    if(meas_eMeasKind_STANDARD == measMode)
    {
      /* standard measurement */
      /* activate the dma controller for data acquisition */
      uint16_t measurementLength = ((2u) * (50u)) / 2;     /* set buffer size (in 32bit) to enable dma for standard measurement */
      
      /* Set EEVAL_DMA_Buffer length */
      ((((DSP_Type *) 0x50000400UL)->EEVAL_DMA_CTRL . bit). length = (uint8_t)(measurementLength));
      
      /* Activate WRAP event see P52417-374 */
      ((((DSP_Type *) 0x50000400UL)->EEVAL_DMA_CTRL . bit). wrap_off = (0u));
      
      loc_pApplRuntimeData->MeasCtrl->Results.MaxMeasurementSamples = measurementLength;
      ((((DSP_Type *) 0x50000400UL)->IRQ_VNO . reg) = (dsp_eInterruptVectorNum_t)(dsp_IRQ_EEVAL_TLH_EVT));
      ((((DSP_Type *) 0x50000400UL)->IRQ_VNO . reg) = (dsp_eInterruptVectorNum_t)(dsp_IRQ_EEVAL_THL_EVT));
      ((((DSP_Type *) 0x50000400UL)->IRQ_VNO . reg) = (dsp_eInterruptVectorNum_t)(dsp_IRQ_EEVAL_MAX_EVT));
      ((((DSP_Type *) 0x50000400UL)->IRQ_VNO . reg) = (dsp_eInterruptVectorNum_t)(dsp_IRQ_EEVAL_MIN_EVT));
    }
    else
    {
      if(meas_eMeasKind_ADVANCED == measMode)
      {
        /* advanced measurement */
        /* activate the dma controller for data acquisition */
        uint16_t measurementLength = ((3u) * (2*(20u))) / 2;   /* set buffer size (in 32bit) to enable dma for advanced measurement */
        ((((DSP_Type *) 0x50000400UL)->EEVAL2_DMA_CTRL . bit). length = (uint8_t)(measurementLength));
        ((((DSP_Type *) 0x50000400UL)->EEVAL1_DMA_CTRL . bit). length = (uint8_t)(measurementLength));
        loc_pApplRuntimeData->MeasCtrl->Results.MaxMeasurementSamples = measurementLength;

        ((((DSP_Type *) 0x50000400UL)->IRQ_VNO . reg) = (dsp_eInterruptVectorNum_t)(dsp_IRQ_EEVAL1_MAX_EVT));
        ((((DSP_Type *) 0x50000400UL)->IRQ_VNO . reg) = (dsp_eInterruptVectorNum_t)(dsp_IRQ_EEVAL2_MAX_EVT));
        ((((DSP_Type *) 0x50000400UL)->IRQ_MASK . reg) |= (1u << (dsp_eInterruptVectorNum_t)(dsp_IRQ_EEVAL1_MAX_EVT)));
        ((((DSP_Type *) 0x50000400UL)->IRQ_MASK . reg) |= (1u << (dsp_eInterruptVectorNum_t)(dsp_IRQ_EEVAL2_MAX_EVT)));
      }
      else
      {
...
void app_VtankMonitoring(Dsi3Drv_pMeasCtrl_t pMeasCtrl)
{
  ...
  {
    /* called before init was executed correctly -> goto error handler */
    err_ErrorHandlerExt(915, ERR_MODLIST_APP);;
  }
}
...
void app_RingingBehaviour(Dsi3Drv_pMeasCtrl_t pMeasCtrl)
{
  if(((void *)0) != pMeasCtrl)
  {
    ...
    if(((1u) == (((DSP_Type *) 0x50000400UL)->RFM_CTRL . bit).rfm_status) && (0 == pMeasCtrl->Results.RingingBehaviour.RingSamples))
    {
      pMeasCtrl->Results.RingingBehaviour.RingCount  = (((DSP_Type *) 0x50000400UL)->RFM_COUNTS . bit).rfm_counts;
      pMeasCtrl->Results.RingingBehaviour.RingSamples= (((DSP_Type *) 0x50000400UL)->RFM_SAMPLES . bit).rfm_samples;
    }
    else
    {
    ...
    if(((1u) == (((DSP_Type *) 0x50000400UL)->RTM . bit).rtm_rt_end) && (0 == pMeasCtrl->Results.RingingBehaviour.RingTime))
    {
      pMeasCtrl->Results.RingingBehaviour.RingTime   = (((DSP_Type *) 0x50000400UL)->RTM . bit).rtm_rt;
      /* Indicate, that ringing behavior measurement has finished */
      pMeasCtrl->Results.RingingBehaviour.AccquisitionFinished = 1;
    }
    else
    {
  ...
  {
    /* called before init was executed correctly -> goto error handler */
    err_ErrorHandlerExt(967, ERR_MODLIST_APP);;
  }
}
...
void app_NearFieldDetection(Dsi3Drv_pMeasCtrl_t pMeasCtrl, uint16_t measurementTime)
{
  if(((void *)0) != pMeasCtrl)
  {
    ...
    if((6000u / (32u)) == measurementTime)
    {
      /* prefetch register values */
      uint16_t loc_DspEcho1 = ((((DSP_Type *) 0x50000400UL)->NFD_ECHO1 . reg));
      uint16_t loc_DspEcho2 = ((((DSP_Type *) 0x50000400UL)->NFD_ECHO2 . reg));
      uint16_t loc_DspEcho3 = ((((DSP_Type *) 0x50000400UL)->NFD_ECHO3 . reg));
      uint16_t loc_DspEchoTime1 = ((((DSP_Type *) 0x50000400UL)->NFD_TS1 . reg));
      uint16_t loc_DspEchoTime2 = ((((DSP_Type *) 0x50000400UL)->NFD_TS2 . reg));
      uint16_t loc_DspEchoTime3 = ((((DSP_Type *) 0x50000400UL)->NFD_TS3 . reg));

      uint16_t nfd_max_num = (((DSP_Type *) 0x50000400UL)->NFD_CTRL . bit).nfd_max_num;

      /* check if first echo has been recorded in time (2ms) */
      if((nfd_max_num > 0) && (loc_DspEcho1 > 0) && (loc_DspEchoTime1 < (2000u)))
      {
        ...
        if((nfd_max_num > 1u) && (loc_DspEcho2 < loc_DspEcho1) && ((uint16_t)(loc_DspEchoTime2 - loc_DspEchoTime1) < (2000u)))
        {
          /* set NFD1 Flag if NFD_CTRL.nfd_irq_cfg = 0 */
          if ( dsp_eNFDIRQ_2_ECHOS == ((dsp_NFDIRQ_Echos_t)(((DSP_Type *) 0x50000400UL)->NFD_CTRL . bit). nfd_irq_cfg) )
          {
            pMeasCtrl->Results.NearFieldDetection.Nfd1Flag = 1;
          }
          else
          {
            /* check next echo */
          ...
          if((loc_DspEcho3 < loc_DspEcho2) && ((uint16_t)(loc_DspEchoTime3 - loc_DspEchoTime2) < (2000u)))
          {
            /* set NFD1 Flag */
            pMeasCtrl->Results.NearFieldDetection.Nfd1Flag = 1;
      ...
      pMeasCtrl->Results.NearFieldDetection.Nfd1Finished = 1;
    }
    else
    {
      /* nothing to do here */
  ...
  {
    /* called before init was executed correctly -> goto error handler */
    err_ErrorHandlerExt(1054, ERR_MODLIST_APP);;
  }
}
...
void app_DeactivateDmaForEchoMeas(void)
{
  ...
  ((((DSP_Type *) 0x50000400UL)->IRQ_MASK . reg) &= ~(1u << (dsp_eInterruptVectorNum_t)(dsp_IRQ_EEVAL_TLH_EVT)));
  ((((DSP_Type *) 0x50000400UL)->IRQ_MASK . reg) &= ~(1u << (dsp_eInterruptVectorNum_t)(dsp_IRQ_EEVAL_THL_EVT)));
  ((((DSP_Type *) 0x50000400UL)->IRQ_MASK . reg) &= ~(1u << (dsp_eInterruptVectorNum_t)(dsp_IRQ_EEVAL_MAX_EVT)));
  ((((DSP_Type *) 0x50000400UL)->IRQ_MASK . reg) &= ~(1u << (dsp_eInterruptVectorNum_t)(dsp_IRQ_EEVAL_MIN_EVT)));
  ((((DSP_Type *) 0x50000400UL)->IRQ_MASK . reg) &= ~(1u << (dsp_eInterruptVectorNum_t)(dsp_IRQ_EEVAL1_MAX_EVT)));
  ((((DSP_Type *) 0x50000400UL)->IRQ_MASK . reg) &= ~(1u << (dsp_eInterruptVectorNum_t)(dsp_IRQ_EEVAL2_MAX_EVT)));
    
  /* disable dma interface */
  ((((DSP_Type *) 0x50000400UL)->EEVAL_DMA_CTRL . bit). length = (uint8_t)(0u));      /* set buffer size to zero, to disable dma */
  ((((DSP_Type *) 0x50000400UL)->EEVAL1_DMA_CTRL . bit). length = (uint8_t)(0u));     /* set buffer size to zero, to disable dma */
  ((((DSP_Type *) 0x50000400UL)->EEVAL2_DMA_CTRL . bit). length = (uint8_t)(0u));     /* set buffer size to zero, to disable dma */
}
/* ****************************************************************************/
...
void loc_SetPdcmAnswerFrameDefault(Dsi3Drv_uPDCMResponseFrame_t* ansData)
{
  ...
  {
    /* called before init was executed correctly -> goto error handler */
    err_ErrorHandlerExt(1108, ERR_MODLIST_APP);;
  }
}
...
void loc_SetPdcmAnswerFrameDiagnostic(Dsi3Drv_uPDCMResponseFrame_t* ansData)
{
  ...
  {
    /* called before init was executed correctly -> goto error handler */
    err_ErrorHandlerExt(1137, ERR_MODLIST_APP);;
  }
}
...
void loc_SetPdcmAnswerFrameBurst(Dsi3Drv_uPDCMResponseFrame_t* ansData)
{
  ...
  {
    /* called before init was executed correctly -> goto error handler */
    err_ErrorHandlerExt(1166, ERR_MODLIST_APP);;
  }
}
...
void loc_SetPdcmAnswerFrameRinging(Dsi3Drv_uPDCMResponseFrame_t* ansData)
{
  ...
  {
    /* called before init was executed correctly -> goto error handler */
    err_ErrorHandlerExt(1194, ERR_MODLIST_APP);;
  }
}
...
void loc_SetPdcmAnswerFrameNfd(Dsi3Drv_uPDCMResponseFrame_t* ansData)
{
  ...
  {
    /* called before init was executed correctly -> goto error handler */
    err_ErrorHandlerExt(1222, ERR_MODLIST_APP);;
  }
}
...
void loc_SetPdcmAnswerFrameStd0(Dsi3Drv_uPDCMResponseFrame_t* ansData, Meas_sEevalStdDataPacketFormat0* loc_BufStd0)
{
  ...
  if((((void *)0) != ansData) && (((void *)0) != loc_BufStd0))
  {
  ...
  {
    /* called before init was executed correctly -> goto error handler */
    err_ErrorHandlerExt(1263, ERR_MODLIST_APP);;
  }
}
...
void loc_SetPdcmAnswerFrameStd1(Dsi3Drv_uPDCMResponseFrame_t* ansData, Meas_sEevalStdDataPacketFormat1*  loc_BufStd1)
{
  ...
  if((((void *)0) != ansData) && (((void *)0) != loc_BufStd1))
  {
  ...
  {
    /* called before init was executed correctly -> goto error handler */
    err_ErrorHandlerExt(1304, ERR_MODLIST_APP);;
  }
}
...
void loc_SetPdcmAnswerFrameAdv(Dsi3Drv_uPDCMResponseFrame_t* ansData, uint8_t frameSelect, Meas_sEevalAdvDataPacketFormat* buffer, uint16_t* tail)
{
  ...
  if((((void *)0) != ansData) && (((void *)0) != buffer) && (((void *)0) != tail)) /* TOF Compensation? siko */
  {
  ...
  {
    /* called before init was executed correctly -> goto error handler */
    err_ErrorHandlerExt(1350, ERR_MODLIST_APP);;
  }
}
...
uint16_t loc_GetRemainingResultCount(Meas_uEevalDataPacket* buffer, uint16_t head, uint16_t* tail, uint16_t minConf, Meas_eMeasurementKind_t kind)
{
  ...
  {
    /* called before init was executed correctly -> goto error handler */
    err_ErrorHandlerExt(1400, ERR_MODLIST_APP);;
  }
...
//      DSP_CTRL_IRQ_DISABLE(dsp_IRQ_EEVAL_MIN_EVT);
  ...
  {
    /* called before init was executed correctly -> goto error handler */
    err_ErrorHandlerExt(1437, ERR_MODLIST_APP);;
  }
}
...
void loc_MeasEvtOccuredInPath2(dsp_eInterruptVectorNum_t irqsrc, dsp_pInterruptContextData_t contextdata)
{
  if(((void *)0) != loc_pApplRuntimeData)
  {
    ...
    {
      loc_pApplRuntimeData->MeasCtrl->Results.MeasRingbufHead2++;
      ((((DSP_Type *) 0x50000400UL)->IRQ_VNO . reg) = (dsp_eInterruptVectorNum_t)(irqsrc));
    }
    else
    {
  ...
  {
    /* called before init was executed correctly -> goto error handler */
    err_ErrorHandlerExt(1469, ERR_MODLIST_APP);;
  }
}
...
uint8_t app_calculate_temperature(uint16_t ada_val_temp)
{
  ...
  tsens = (40) + (25) + (( (int16_t)(ada_val_temp - ((*((uint16_t*)((0x48000000u) + 0xF8u)))&0x03FFu)) * tsense_calib_eff)>>10);
