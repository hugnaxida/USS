...
void syss_Init(void)
{
  ...
  (((SYS_STATE_Type *) 0x40000100UL)->CONTROL . bit . sys_clk_sel = (syss_SysClockFrequency_t)(syss_CLockSel_24MHZ));  
  
  /* Initialize VIC */
    InterruptHandlerInitialize(&nvic_InterfaceFunctions);

  /* Set module priority level */
  nvic_InterfaceFunctions.SetModulePriority(nvic_CEVT_EXT_IRQ_0, nvic_Priority_Level_0);
  
  /* Initialize SWT IRQ */
  system_ctrl_InterruptInitialisation(&nvic_InterfaceFunctions, &sysctrl_envData, ((void *)0));
  
  /* standby request flag */
  ChangeStandbyState = 0;
  ...
  sys_state_InterruptInitialisation(&nvic_InterfaceFunctions, &sysstate_envData, ((void *)0));
...
void syss_ExitSleep(void)
{
  ...
  (((SYSTEM_CTRL_Type *) 0x50000500UL)->POWER . bit).standby = 0u;
  /* disable dsi3 wake up event interrupt */
  system_ctrl_InterruptDisable(system_ctrl_IRQ_DSI3_WAKE_UP);
  /* switch DSP on */
  (((DSP_Type *) 0x50000400UL)->DSP_CTRL . bit).dsp_on = 1u; /*SMA52417-142*/
}
...
void syss_ProcessStandbyRequests(void)
{
  if(1 == ChangeStandbyState)
  {
    ...
    (((DSP_Type *) 0x50000400UL)->DSP_CTRL . bit).dsp_pwr_dwn = 1;

    while(1  == (((DSP_Type *) 0x50000400UL)->DSP_CTRL . bit).dsp_on_state)
    {
      /* wait...*/
    }

    (((SYSTEM_CTRL_Type *) 0x50000500UL)->POWER . bit).standby = 1;
