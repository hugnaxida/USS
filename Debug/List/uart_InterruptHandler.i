...
typedef struct {                                    /*!< DSI3 Structure                                                        */
  ...
  union {
    ...
    struct {
      ...
      volatile uint16_t  rcc_slope  :  3;               /*!< rcc_slope bitfield                                                    */
      volatile uint16_t  rcc_curve  :  1;               /*!< rcc_curve bitfield                                                    */
      volatile uint16_t  rcc_cur_ofs:  3;               /*!< rcc_cur_ofs bitfield                                                  */
      ...
      volatile uint16_t  fcc_debounce:  3;              /*!< fcc_debounce bitfield                                                 */
    } bit;                                          /*!< BitSize                                                               */
  } ANALOG_CTRL;
  ...
  union {
    ...
    struct {
      ...
      volatile uint16_t  t_bit_fcc  :  2;               /*!< t_bit_fcc bitfield                                                    */
    } bit;                                          /*!< BitSize                                                               */
  } CONFIG;
  ...
  union {
    volatile  uint16_t  reg;                             /*!< RCC_DAC register                                                      */
  ...
  union {
    volatile const  uint16_t  reg;                             /*!< HW_VERSION register                                                   */
    
    struct {
      volatile const  uint16_t  value      :  8;               /*!< value bitfield                                                        */
    } bit;                                          /*!< BitSize                                                               */
  } HW_VERSION;
  volatile const  uint16_t  RESERVED4;
 ...
 * to swtimer_SetIRQVEnable, that a special enum (uart_eInterruptVectorNum_t)
...
void uart_InterruptEnable(uart_eInterruptVectorNum_t irqsrc)
{
  (((UART_Type *) 0x5F000000UL)->IRQ_VENABLE . reg) = (uint16_t) irqsrc;  
}
/***************************************************************************//**
 * @implementation
 * Simple word write access to IRQ_VDISABLE register, but with the difference
 * to swtimer_SetIRQVDisable, that a special enum (uart_eInterruptVectorNum_t)
 * is passed as parameter, for security reasons and a more concrete interface
 * definition.
 *
 ******************************************************************************/
void uart_InterruptDisable(uart_eInterruptVectorNum_t irqsrc)
{
  (((UART_Type *) 0x5F000000UL)->IRQ_VDISABLE . reg) = (uint16_t) irqsrc;
}
/***************************************************************************//**
 * @implementation
...
{
  // PRQA S 0316 1
  uart_sInterruptEnvironmentData_t* evironmentData = (uart_sInterruptEnvironmentData_t*) nvic_GetPointerToEnvironmentData(nvic_CEVT_EXT_IRQ_6);

  if ((irqsrc < uart_INTERRUPT_VECTOR_CNT) && (evironmentData != ((void *)0)))
  {
...
/***************************************************************************//**
 * @implementation
 * Reverse function of "uart_RegisterInterruptCallback". Sets the corresponding
...
void uart_InterruptDeregisterCallback(uart_eInterruptVectorNum_t irqvecnum)
{
  ...
  if ((irqvecnum < uart_INTERRUPT_VECTOR_CNT) && (evironmentData != ((void *)0)))
  {
    uart_InterruptDisable(irqvecnum);
    evironmentData->InterrupVectorTable[irqvecnum] = ((void *)0);
  }
  else {}
}
 ...
 ******************************************************************************/
 static __irq void loc_InterruptHandler(void)
{
  uart_eInterruptVectorNum_t irqvecnum = (uart_eInterruptVectorNum_t) (((UART_Type *) 0x5F000000UL)->IRQ_VNO . reg);
  
  if (irqvecnum < uart_INTERRUPT_VECTOR_CNT) /* ensure the IRQ trigger is not already gone away until processing starts */
  {

    // PRQA S 0316 1
    uart_sInterruptEnvironmentData_t* evironmentData = (uart_sInterruptEnvironmentData_t*) nvic_GetPointerToEnvironmentData(nvic_CEVT_EXT_IRQ_6);

    uart_InterruptCallback_t fptr;
...
//      __disable_interrupt();
    ...
    else 
    {
      ...
      (((UART_Type *) 0x5F000000UL)->IRQ_VDISABLE . reg) = (uint16_t) irqvecnum;
    }
      
    /* Clear interrupt request flag */
    (((UART_Type *) 0x5F000000UL)->IRQ_VNO . reg) = (uint16_t) irqvecnum;
...
void uart_InterruptInitialisation(nvic_cpInterfaceFunctions_t vicIf, uart_pInterruptEnvironmentData_t environmentdata, uart_pInterruptContextData_t contextdata)
{  
  if ((vicIf != (const void *) ((void *)0)) && (vicIf->InterfaceVersion == 0x0102) && (environmentdata != ((void *)0)))
  {
    environmentdata->ContextData = contextdata;

    /* Register module at interrupt handler */
    vicIf->RegisterModule(nvic_CEVT_EXT_IRQ_6, &loc_InterruptHandler, environmentdata);
    vicIf->EnableModule(nvic_CEVT_EXT_IRQ_6);
  }
  else {}
