...
void ada_Init(void)
{ 
  ((((ADA_Type *) 0x50000700UL)->ADA_CFG . bit). adc_cdiv = (ada_ClockDiv_t)((1u)));
  ((((ADA_Type *) 0x50000700UL)->ADA_CFG . bit). adc_tsamp = (ada_SamplingPeriod_t)((1u)));
}
...
uint16_t ada_TriggerSample(ada_ChannelSelect_t ch, ada_SignalSelect_t sig)
{
  ...
  ((((ADA_Type *) 0x50000700UL)->ADA_CFG . bit). chsel = (ada_ChannelSelect_t)(ch));
  ((((ADA_Type *) 0x50000700UL)->ADA_CFG . bit). sigsel = (ada_SignalSelect_t)(sig));

  for (uint8_t i=0; i < sampleCount;++i)
  {
    if (ada_eConversion_RUNNING != ((ada_ProgressStatus_t)(((ADA_Type *) 0x50000700UL)->ADA_CTRL . bit). eoc))
    {
      ((((ADA_Type *) 0x50000700UL)->ADA_CTRL . bit). soc = (1u));
      
      /* wait for analog data aquisition finished */
      while(ada_eConversion_FINISHED != ((ada_ProgressStatus_t)(((ADA_Type *) 0x50000700UL)->ADA_CTRL . bit). eoc))
      {};
      retVal += ((((ADA_Type *) 0x50000700UL)->ADC_DTA . bit). adc_dta); 
    }
  }
